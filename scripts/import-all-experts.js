// Script to import all expert JSON files from public/expertbackup to MongoDB
...
async function importAllExperts() {
  console.log('Starting comprehensive expert import from JSON files...');
  ...
  try {
    ...
    const allFiles = fs.readdirSync(backupDir).filter(file => file.endsWith('.json'));
    ...
    console.log(`Found ${allFiles.length} JSON files in the backup directory:`);
    allFiles.forEach(file => console.log(` - ${file}`));
    
    // First process sample-experts.json if it exists
    const sampleFile = allFiles.find(file => file === 'sample-experts.json');
    const regularFiles = allFiles.filter(file => file !== 'sample-experts.json');
    
    // Process files in the right order: first sample file, then other files
    const orderedFiles = sampleFile ? [sampleFile, ...regularFiles] : regularFiles;
    ...
    for (const file of orderedFiles) {
      ...
      console.log(`\nProcessing file ${processedFiles + 1}/${orderedFiles.length}: ${file}`);
    ...
    console.log('\nImport Summary:');
    console.log(`Total files processed: ${processedFiles}/${orderedFiles.length}`);
...
importAllExperts().catch(console.error);